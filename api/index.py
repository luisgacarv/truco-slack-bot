...
def process_gemini_request(slack_text, response_url):
    try:
...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        # Resposta imediata para evitar timeout do Slack
        self.send_response(200)
        self.send_header('Content-type', 'application/json')
        self.end_headers()
        self.wfile.write(
            '{"response_type": "ephemeral", "text": "Processando sua solicitação..."}'
            .encode('utf-8')
        )

        # Inicia a thread para processar a requisição do Gemini em segundo plano
        thread = threading.Thread(target=process_gemini_request, args=(slack_text, response_url))
        thread.start()
